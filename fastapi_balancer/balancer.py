...
from .middleware import BalancerMiddleware
...
class Balancer:
    ...
    def wrap(self, app: FastAPI, endpoints: list[str]) -> None:
        cfg = self._config
        original_lifespan = app.router.lifespan_context

        logger.info("fastapi-balancer initializing — endpoints=%s strategy=%s storage=%s",
                    endpoints, cfg.strategy, cfg.storage or "memory")

        storage = get_storage(cfg.storage)
        http_client = httpx.AsyncClient()
        prober = Prober(storage, http_client, cfg.error_threshold, cfg.latency_threshold_ms)
        health_checker = HealthChecker(storage, app)
        router = get_router(cfg.strategy, storage)
        limiter = Limiter(storage)

        logger.info("middleware registered for %d endpoint(s): %s", len(endpoints), endpoints)
        ...
        async def lifespan(a: FastAPI):
            ...
            if original_lifespan:
                async with original_lifespan(a):
                    await self._startup(
                        app, endpoints, cfg, storage, http_client,
                        prober, health_checker, probe_base_url,
                    )
                    yield
        ...
        if ui_dist.is_dir():
            app.mount("/balancer/ui/assets", StaticFiles(directory=str(ui_dist / "assets")), name="balancer_assets")
            ...
            def _check_basic_auth(authorization: str | None) -> bool:
                ...
                try:
                    ...
                    return False
                return (
                    secrets.compare_digest(user, ui_username or "")
                    and secrets.compare_digest(pwd, ui_password or "")
                )

            @app.get("/balancer/ui", include_in_schema=False)
            @app.get("/balancer/ui/{_:path}", include_in_schema=False)
    ...
    async def _startup(self, app, endpoints, cfg, storage, http_client, prober, health_checker, base_url):
        logger.info("balancer startup — probe_on_startup=%s queue_timeout=%.1fs health_check_interval=%ds",
                    cfg.probe_on_startup, cfg.queue_timeout, cfg.health_check_interval)
        ...
        for endpoint in endpoints:
            ...
            if probe_cfg is not None and probe_cfg.capacity is not None:
                ...
                logger.info("[capacity] %s — manual capacity=%d", endpoint, probe_cfg.capacity)

        if cfg.probe_on_startup:
            if cfg.force_reprobe:
                logger.info("force_reprobe=True — clearing cached capacity from storage")
                for endpoint in endpoints:
                    probe_cfg = cfg.endpoints.get(endpoint)
            ...
            logger.info("starting throughput probing for %d endpoint(s) ...", len(endpoints))
            ...
            for endpoint in endpoints:
                ...
                if probe_cfg.capacity is not None:
                    logger.info("[probe] %s — skipping probe, manual capacity=%d already set", endpoint, probe_cfg.capacity)
                    continue
                ...
                result = await prober.probe(base_url, endpoint, probe_cfg, force=cfg.force_reprobe, client=probe_client)
                logger.info(
                    "[probe] %s — max_concurrency=%d p99=%.1fms rps=%.1f",
                    endpoint, result.max_concurrency, result.p99_latency_ms, result.rps,
                )
            await probe_client.aclose()
            logger.info("probing complete for all endpoints")
        else:
            logger.warning("probe_on_startup=False — capacity limits not set, all requests will pass through")

        logger.info("starting health checker — endpoint=%s interval=%ds",
                    cfg.health_endpoint, cfg.health_check_interval)
        ...
        for attempt in range(int(max_wait / interval)):
            try:
                ...
                if resp.status_code < 500:
                    ...
                    return
                logger.info("waiting for %s — got status=%d, retrying in %.0fs ...", endpoint, resp.status_code, interval)
            except Exception as e:
                logger.info("waiting for %s — %s, retrying in %.0fs ...", endpoint, e, interval)
            await asyncio.sleep(interval)
        logger.warning("gave up waiting for %s after %ds — probing anyway", endpoint, max_wait)
